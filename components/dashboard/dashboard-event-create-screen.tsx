...
export function DashboardEventCreateScreen() {
  const { activeTab, newEvent, setNewEvent, handleCreateEvent, setActiveTab, editingEvent, uploadingEventImage, handleEventImageUpload } = useDashboardContext()
    ...
    <section className="mx-auto w-full max-w-5xl">
      ...
      <form onSubmit={handleCreateEvent} className="overflow-hidden rounded-[2rem] border border-foreground/10 bg-card shadow-sm">
        ...
        <div className="space-y-8 px-6 py-8 sm:px-10">
          ...
          <div><SectionTitle>Fecha y ubicación</SectionTitle><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3"><Field label="Fecha *" value={newEvent.eventDate} onChange={(value) => update({ eventDate: value })} type="date" required /><Field label="Hora" value={newEvent.eventTime} onChange={(value) => update({ eventTime: value })} type="time" /><Field label="Dirección *" value={newEvent.location} onChange={(value) => update({ location: value })} placeholder="Calle, número, colonia y ciudad" required /></div><div className="mt-5"><EventLocationPicker latitude={newEvent.latitude} longitude={newEvent.longitude} onChange={(location) => update({ latitude: location.latitude, longitude: location.longitude })} /></div></div>
          <div><SectionTitle>Contacto y registro</SectionTitle><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3"><Field label="Meta de asistentes" value={String(newEvent.attendeesTarget)} onChange={(value) => update({ attendeesTarget: Number.parseInt(value, 10) || 0 })} type="number" /><Field label="Responsable" value={newEvent.contactName} onChange={(value) => update({ contactName: value })} placeholder="Nombre del contacto" /><Field label="Teléfono" value={newEvent.contactPhone} onChange={(value) => update({ contactPhone: value })} placeholder="555 000 0000" /><Field label="Liga de registro" value={newEvent.registrationLink} onChange={(value) => update({ registrationLink: value })} placeholder="https://..." /></div></div>
