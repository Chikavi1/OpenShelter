...
export function EventDetailClient({ id }: { id: string }) {
  ...
  return (
    <PublicPageShell appName={appName} contentClassName="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
        ...
        <section className="overflow-hidden rounded-[2rem] bg-card shadow-sm sm:rounded-[2.5rem] sm:bg-secondary">
          <div className="relative">
            <img src={cover} alt={event.title} className="h-[220px] w-full object-cover sm:h-[460px]" />
            <div className="absolute inset-0 hidden bg-gradient-to-t from-black/75 via-black/20 to-transparent sm:block" aria-hidden="true" />
            <div className="absolute inset-x-5 bottom-5 hidden text-white sm:inset-x-10 sm:bottom-10 sm:block"><div className="flex flex-wrap gap-2"><span className="rounded-full bg-white/15 px-3 py-1.5 text-xs font-semibold uppercase tracking-wider backdrop-blur">{event.category}</span><span className="rounded-full bg-accent px-3 py-1.5 text-xs font-semibold text-accent-foreground">{event.status}</span></div><h1 className="mt-4 max-w-3xl text-4xl font-semibold tracking-[-0.06em] sm:text-6xl">{event.title}</h1></div>
          </div>
          <div className="p-5 sm:hidden">
            <div className="flex flex-wrap gap-2"><span className="rounded-full border border-foreground/10 bg-secondary px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wider">{event.category}</span><span className="rounded-full bg-accent px-3 py-1.5 text-xs font-semibold text-accent-foreground">{event.status}</span></div>
            <h1 className="mt-3 text-[22px] font-semibold leading-tight tracking-[-0.04em]">{event.title}</h1>
          </div>
        </section>
