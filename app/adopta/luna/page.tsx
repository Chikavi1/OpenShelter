...
export default function LunaProfilePage() {
  ...
  const pet = site.pets.find((item) => item.name.toLowerCase() === 'luna')
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-10">
        ...
        <section className="grid gap-8 py-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-start lg:gap-16 lg:py-16">
          ...
          <div className="lg:pt-3"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Conoce a tu nuevo mejor amigo</p><h1 className="mt-4 text-6xl font-semibold tracking-[-0.07em] sm:text-8xl">{pet?.name || 'Luna'}<span className="text-muted-foreground"></span></h1><p className="mt-5 max-w-lg text-lg leading-8 text-muted-foreground">{pet ? `${pet.breed} dulce y tranquila que elige a su persona para ronronear y dormir la siesta.` : 'Una compañera lista para encontrar un hogar.'}</p><div className="mt-9 grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-foreground/10 bg-foreground/10 sm:grid-cols-4"><Stat label="Edad" value={pet?.age || '1 año'} /><Stat label="Tamaño" value={pet?.size || 'Pequeño'} /><Stat label="Sexo" value={pet?.gender || 'Hembra'} /><Stat label="Ubicación" value={pet?.location || 'CDMX'} /></div><div className="mt-9 flex flex-wrap gap-2">{(pet?.personality || ['Mimosota', 'Curiosa', 'Tranquila']).slice(0, 4).map((tag) => <span key={tag} className="rounded-full border border-foreground/15 px-4 py-2 text-sm">{tag}</span>)}</div><div className="mt-10 border-t border-foreground/10 pt-8"><h2 className="text-2xl font-semibold">Su historia</h2><p className="mt-3 leading-7 text-muted-foreground">{pet?.story || 'Luna está lista para encontrar un hogar.'}</p></div><div className="mt-8 flex items-center gap-3 text-sm text-muted-foreground"><ShieldCheck className="size-5 text-accent-foreground" /> {pet?.health?.join(' · ') || 'Esterilizada, vacunada y con seguimiento veterinario'}</div></div>
        </section>
        ...
        <section className="rounded-[2rem] border border-foreground/10 bg-card p-6 sm:p-10">
          ...
          <ul className="mt-10 grid gap-x-12 gap-y-6 lg:grid-cols-2 lg:gap-x-16">
            {['Ser mayor de 18 años y presentar una identificación oficial vigente', 'Contar con un espacio seguro y adecuado para una gata pequeña', 'Disponer de tiempo, recursos económicos y compromiso para su cuidado', 'Aceptar el compromiso de esterilización y el seguimiento veterinario', 'Firmar el contrato de adopción y comprometerse a no abandonarla', 'Permitir una visita de seguimiento durante los primeros meses'].map((requirement, index) => <li key={requirement} className="flex items-start gap-4 text-sm leading-6"><span className="grid size-8 shrink-0 place-items-center rounded-full bg-accent text-accent-foreground">{index + 1}</span> <span className="pt-0.5">{requirement}</span></li>)}
          </ul>
        </section>
