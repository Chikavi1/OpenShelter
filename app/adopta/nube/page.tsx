...
export default function NubeProfilePage() {
  ...
  const pet = site.pets.find((item) => item.name.toLowerCase() === 'nube')
    ...
    return (
      <main className="min-h-screen bg-background text-foreground">
        <div className="mx-auto max-w-3xl px-4 py-20 text-center sm:px-6 lg:px-10">
          ...
          <a href="/catalogo" className="mt-8 inline-flex items-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-medium text-primary-foreground">
            Ver catálogo
          </a>
        </div>
      </main>
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-10">
        ...
        <section className="grid gap-8 py-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-start lg:gap-16 lg:py-16">
          <div>
            <div className="relative overflow-hidden rounded-[2rem] bg-secondary"><img src={photos[activePhoto] || pet?.image} alt={`${pet?.name || 'Mascota'} en adopción`} className="aspect-[1.05] w-full object-cover sm:aspect-[1.15]" /><button aria-label="Guardar a Nube" className="absolute right-5 top-5 grid size-12 place-items-center rounded-full bg-background/90 backdrop-blur transition hover:scale-105"><Heart className="size-5" /></button><span className="absolute bottom-5 left-5 rounded-full bg-accent px-4 py-2 text-xs font-semibold uppercase tracking-wider text-accent-foreground">Disponible para adopción</span></div>
            <div className="mt-4 grid grid-cols-3 gap-3">{photos.map((photo, index) => <button key={photo} onClick={() => setActivePhoto(index)} className={`overflow-hidden rounded-2xl transition ${activePhoto === index ? 'ring-2 ring-primary ring-offset-2 ring-offset-background' : 'opacity-65 hover:opacity-100'}`}><img src={photo} alt={`Foto ${index + 1} de ${pet?.name || 'la mascota'}`} className="aspect-square w-full object-cover" /></button>)}</div>
          </div>
          <div className="lg:pt-3"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Conoce a tu nuevo mejor amigo</p><h1 className="mt-4 text-6xl font-semibold tracking-[-0.07em] sm:text-8xl">{pet?.name || 'Nube'}<span className="text-muted-foreground"></span></h1><p className="mt-5 max-w-lg text-lg leading-8 text-muted-foreground">{pet ? `${pet.breed} aventurero y sociable que convierte cualquier caja en un castillo.` : 'Un compañero listo para encontrar un hogar.'}</p><div className="mt-9 grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-foreground/10 bg-foreground/10 sm:grid-cols-4"><Stat label="Edad" value={pet?.age || '8 meses'} /><Stat label="Tamaño" value={pet?.size || 'Pequeño'} /><Stat label="Sexo" value={pet?.gender || 'Macho'} /><Stat label="Ubicación" value={pet?.location || 'Puebla'} /></div><div className="mt-9 flex flex-wrap gap-2">{(pet?.personality || ['Travieso', 'Aventurero', 'Sociable']).slice(0, 4).map((tag) => <span key={tag} className="rounded-full border border-foreground/15 px-4 py-2 text-sm">{tag}</span>)}</div><div className="mt-10 border-t border-foreground/10 pt-8"><h2 className="text-2xl font-semibold">Su historia</h2><p className="mt-3 leading-7 text-muted-foreground">{pet?.story || 'Nube está listo para encontrar un hogar.'}</p></div><div className="mt-8 flex items-center gap-3 text-sm text-muted-foreground"><ShieldCheck className="size-5 text-accent-foreground" /> {pet?.health?.join(' · ') || 'Esterilizado, vacunado y con seguimiento veterinario'}</div></div>
        </section>

        <VaccinationHistory
          petName="Nube"
          species="Gato"
          highlights={pet?.health?.slice(0, 3) ?? []}
