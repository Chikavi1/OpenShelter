...
export async function GET() {
  ...
  if (logoUrl) {
    try {
      // Si es un upload local/S3 (/uploads/xxx), leer directo del storage sin hacer fetch HTTP a sí mismo
  ...
  // Fallback: archivo local cuadrado (siempre visible, sin depender de fetch HTTP)
  try {
  ...
  try {
    const fallback = path.join(process.cwd(), 'public', 'placeholder-logo.png')
