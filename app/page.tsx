    ...
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      <div className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-10">
        ...
        <section id="inicio" className="grid gap-10 py-14 md:py-20 lg:grid-cols-[1fr_0.9fr] lg:items-end lg:gap-20">
          ...
          <div className="relative"><div className="overflow-hidden rounded-[2rem] bg-secondary"><img src={HERO_IMAGE} alt="Grupo de perros rescatados buscando un hogar" className="h-[390px] w-full object-cover object-bottom mix-blend-multiply sm:h-[500px]" /></div><div className="absolute -bottom-5 -left-3 rounded-2xl bg-accent p-4 shadow-xl sm:-left-6"><p className="text-3xl font-semibold tracking-tight">16</p><p className="text-xs font-medium uppercase tracking-wider">vidas transformadas</p></div></div>
        </section>

        <section id="adopta" className="scroll-mt-10 py-20"><div className="mb-10 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="mb-3 text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Encuentra a tu match</p><h2 className="text-4xl font-semibold tracking-[-0.05em] sm:text-5xl">Conoce a nuestros rescatados</h2></div><div className="flex items-center gap-2 rounded-full border border-foreground/10 bg-card p-1 text-sm"><Search className="ml-3 size-4 text-muted-foreground" />{['Todos', 'Perro', 'Gato'].map((item) => <button key={item} onClick={() => setFilter(item)} className={`rounded-full px-4 py-2 transition ${filter === item ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}>{item}</button>)}</div></div><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{filteredPets.map((pet) => <a key={pet.id} href={`/adopta/${pet.slug}`} className="group block"><div className={`relative overflow-hidden rounded-3xl ${pet.tone}`}><img src={pet.image} alt={`${pet.name}, ${pet.breed} en adopción`} className="aspect-[0.86] w-full object-cover transition duration-500 group-hover:scale-105" /><span className="absolute bottom-4 left-4 rounded-full bg-background/90 px-3 py-1 text-xs font-medium">Disponible</span></div><div className="flex items-start justify-between gap-3 px-1 pt-4"><div><h3 className="text-xl font-semibold transition group-hover:text-primary">{pet.name}</h3><p className="mt-1 text-sm text-muted-foreground">{pet.breed} · {pet.age}</p></div><p className="pt-1 text-xs text-muted-foreground">{pet.location}</p></div></a>)}</div></section>

        <section className="grid gap-8 rounded-[2rem] bg-primary p-6 text-primary-foreground sm:p-10 lg:grid-cols-[0.8fr_1.2fr] lg:p-14"><div className="flex flex-col justify-between"><div><p className="mb-4 text-xs font-semibold uppercase tracking-[0.2em] text-primary-foreground/60">Perfil destacado</p><h2 className="max-w-md text-4xl font-semibold tracking-[-0.05em] sm:text-6xl">Milo está listo para conocerte.</h2><p className="mt-6 max-w-sm leading-7 text-primary-foreground/70">Cariñoso, juguetón y experto en hacer que cualquier día se sienta especial.</p></div><a href="/adopta/milo" className="mt-10 w-fit rounded-full bg-accent px-5 py-3 text-sm font-medium text-accent-foreground">Quiero conocer a Milo <ArrowDownRight className="ml-2 inline size-4" /></a></div><div className="grid gap-4 sm:grid-cols-[1.1fr_0.9fr]"><img src="https://images.unsplash.com/photo-1558788353-f76d92427f16?auto=format&fit=crop&w=1000&q=85" alt="Milo, perro rescatado" className="h-full min-h-72 w-full rounded-3xl object-cover" /><div className="flex flex-col justify-between rounded-3xl bg-primary-foreground/10 p-6"><div className="flex flex-wrap gap-2 text-xs"><span className="rounded-full border border-primary-foreground/20 px-3 py-1">2 años</span><span className="rounded-full border border-primary-foreground/20 px-3 py-1">Mediano</span><span className="rounded-full border border-primary-foreground/20 px-3 py-1">Macho</span></div><div><p className="mb-2 text-xs uppercase tracking-wider text-primary-foreground/50">Personalidad</p><p className="text-2xl font-medium">Leal · curioso · noble</p></div><div className="border-t border-primary-foreground/15 pt-4 text-sm text-primary-foreground/65">Milo convive con otros perros y disfruta los paseos largos.</div></div></div></section>
